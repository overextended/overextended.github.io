# Common Issues (/docs/ox_inventory/issues)
...
To do so please read our [*Installation*](../ox_inventory#installation) guide.

## No such export \* in resource ox\_inventory [#no-such-export--in-resource-ox_inventory]
...
* The resource trying to use the export (e.g. esx*addoninventory) is being started \_before* ox\_inventory.
* You're literally trying to call an export that does not exist, which is a you issue.

## Stashes / trunks are not saved at server restart [#stashes--trunks-are-not-saved-at-server-restart]
