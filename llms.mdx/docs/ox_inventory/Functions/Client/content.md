# Client (/docs/ox_inventory/Functions/Client)
...
<Tabs items="['Player', 'Shop', 'Stash', 'Crafting', 'Police Evidence', 'Glovebox / Trunk']">
  <Tab>
  ...
  <Tab>
    Opens the [second location](https://github.com/overextended/ox_inventory/blob/main/data/crafting.lua#L17) of the debug\_crafting recipe.

    ```lua
    exports.ox_inventory:openInventory('crafting', { id = 'debug_crafting', id = 2 })
    ```
  </Tab>

  <Tab>
    Opens the number 1 evidence locker.

    ```lua
    exports.ox_inventory:openInventory('policeevidence', 1)
    ```

    Opens a dialog to select the evidence locker.
  ...
  <Tab>
    Opens the glovebox of a vehicle.

    ```lua
    local netid = NetworkGetNetworkIdFromEntity(cache.vehicle)
    exports.ox_inventory:openInventory('glovebox', { netid = netid })
    ```
...
## GetSlotIdWithItem [#getslotidwithitem]
...
## GetSlotsIdWithItem [#getslotsidwithitem]
...
## GetSlotWithItem [#getslotwithitem]
...
## supressItemNotifications [#supressitemnotifications]
...
## setGetPlayerNameMethod [#setgetplayernamemethod]
...
#### Disable opening inventory [#disable-opening-inventory]

```lua
LocalPlayer.state.invBusy = true
```

### invHotkeys [#invhotkeys]
