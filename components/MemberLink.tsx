...
const MemberLink: React.FC<{ image: string; name: string; sponsor?: string }> = ({
  image,
  name,
  sponsor,
}) => {
  return (
    <div key={name} className="flex flex-col items-center justify-center gap-2">
      ...
      <a href={sponsor ? `https://ko-fi.com/${sponsor}` : ""} target="_blank" className="w-full">
        <button
          disabled={!sponsor}
          className="flex w-full items-center justify-center gap-2 rounded-md p-2 text-sky-500 bg-sky-500/20 hover:bg-sky-500/30 disabled:text-slate-500 disabled:bg-slate-500/20"
        >
          <IconCoffee />
          Support
        </button>
