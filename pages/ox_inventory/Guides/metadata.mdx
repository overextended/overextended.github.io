...
We can easily create metadata items by defining a hook using [createItem](../Functions/Server/Hooks#creatitem) and assign
the metadata we want by adding a `type` to the item in the shop inventory.
