...
- data: `table` (`object`)
  ...
  - actions?: `table` (`array`)
    - id: `string`
    - label: `string`
    - variant?: `'filled'` or `'outline'` or `'light'` or `'white'` or `'default'` or `'subtle'` or `'gradient'`
...
When the user clicks on a custom button, the returned value is the ID of the action.
